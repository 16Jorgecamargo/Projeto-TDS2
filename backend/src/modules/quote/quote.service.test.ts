...
describe('QuoteService', () => {
  ...
  describe('create', () => {
    it('grava o total informado com 2 casas', async () => {
    ...
    it('lanca NotFoundError quando demanda nao existe', async () => {
      ...
      await expect(
        service.create('pro-1', 'demand-x', { message: 'orcamento valido', validUntil: null, total: 10 }),
    ...
    it('rejeita orcamento em demanda nao aberta', async () => {
      ...
      await expect(
        service.create('pro-1', 'demand-1', { message: 'orcamento valido', validUntil: null, total: 10 }),
  ...
  describe('listByDemand', () => {
    it('retorna orcamentos da demanda', async () => {
  ...
  describe('listMinePending', () => {
    it('retorna apenas orcamentos pendentes do profissional com titulo da demanda', async () => {
      quotes.find.mockResolvedValueOnce([
        {
          id: 'quote-1',
          demand_id: 'demand-1',
          ...
          created_at: new Date('2026-07-01T12:00:00Z'),
          updated_at: new Date('2026-07-01T12:00:00Z'),
          demand: { title: 'Instalacao eletrica' },
        } as Quote,
      ]);

      const result = await service.listMinePending('pro-1');

      expect(quotes.find).toHaveBeenCalledWith(
        expect.objectContaining({ where: { professional_id: 'pro-1', status: 'pending' } }),
      );
      expect(result).toHaveLength(1);
      expect(result[0]?.demandTitle).toBe('Instalacao eletrica');
    });
  });
