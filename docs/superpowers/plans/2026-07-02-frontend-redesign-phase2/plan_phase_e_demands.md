...
### Task 14: Verificação visual interativa (Tasks 1-13)

Não é uma task de subagent — o controller (sessão principal) faz isso pessoalmente com o Playwright MCP interativo, o mesmo fluxo usado ao final da Fase 1: subir o stack, navegar pelas telas construídas nas Tasks 1-13 como um usuário real, tirar screenshot de cada uma, e corrigir na hora qualquer bug visual real encontrado (responsividade, overflow, token de cor errado, etc.) via um fix dispatch pontual — não esperar um relatório de subagent para depois revisar.

Telas a cobrir: Dashboard Cliente (`/`, autenticado como client), Busca (`/search`, filtros + cards), fluxo de Publicar Demanda com foto (`/demands/new`), Detalhe de Demanda (`/demands/:id`), Lista de Demandas (`/demands`), Perfil Público de um profissional (`/professionals/:id`, header + portfólio + disponibilidade + avaliações). Testar pelo menos desktop e um breakpoint mobile (375px), já que a Fase 1 achou um bug real só visível em mobile.

Pré-requisito: stack completo rodando (`docker compose up -d mysql redis`, backend migrado com `npm run dev --workspace @marketplace/backend`, frontend com `npm run dev --workspace @marketplace/frontend`), e pelo menos um usuário cliente e um profissional com perfil público existentes (registrar via UI ou usar dados já seedados).
